...
litero.prototype = function () {
    ...
    inner.validate = function (options, callback) {
        ...
        story.url = options.url.replace(/(^|\/)(www\.)?literotica/, '$1classic.literotica');

        url = /^(?:https?\:\/\/)?(?:w{3}\.)?(?:i\.)?(classic.)?(literotica\.com)(\/s(?:tories)?\/(?:showstory\.php\?(?:url|id)=)?([a-z-0-9]+))$/.exec(story.url);

        if (url) {
            story.request = story.request || {};
            story.request.path = url[3];
            story.request.host = url[1] + url[2];
            story.request.headers = story.request.headers || {};
            story.request.headers['Cookie'] = 'enable_classic=1; ' + (story.request.headers['Cookie'] || '');

            story.filename = story.filename || url[4];
